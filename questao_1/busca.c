...
void buscaProfundidade(Grafo *gr, int ini, int *visitado, int cont, int *caminho, int fim){
    ...
    for(i=0; i<gr->grau[ini]; i++){
        if(visitado[gr->arestas[ini][i]]==0){
            ...
            if(v==0)
                caminho[cont]=gr->arestas[ini][i];
            if(gr->arestas[ini][i] == fim){
                break;
            }
            buscaProfundidade(gr,gr->arestas[ini][i],visitado,cont+1,caminho, fim);
        }
    }
}

void buscaProfundidade_Grafo(Grafo *gr, int ini, int *visitado,int fim){
    int i, cont = 1 , caminho[37];
    for(i=0; i<gr->nro_vertices; i++){
        caminho[i] = 0;
        visitado[i] = 0;
    }
    ...
    buscaProfundidade(gr,ini,visitado,cont,caminho, fim);

    printf("o caminho do labirinto e ");
    for(i=0; i < gr->nro_vertices; i++){
        if(caminho[i]!=0)
            printf(" %d ",caminho[i]);
    }
